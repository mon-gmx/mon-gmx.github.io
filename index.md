...
So trying to be the simplest, these computers use their qubits to represent all possible answers (all possible combinations in a word) at the same time. Qubits may contain both states of information (_superposition_) so in 2 quibits we would have a word that is both 00, 01, 10 and 11. To make it even more intriguing, qubits often are entangled, thus, these 2 bits will contain all combinations when you only need to measure one to infer the values on the other. To make this even more interesting, you can have way larger and more complex words with less qubits compared to digital representation of bits, since the words are 2^N (N=number of qubits) unline binary words where you can only represent N bits in a word.
